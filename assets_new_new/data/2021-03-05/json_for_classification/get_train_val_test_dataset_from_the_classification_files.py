...
def get_image_label_list_from_origignal_annotation(image_file_path,txt_save_folder,label_mapping):
...
def get_train_val_test_dataset_from_the_classification_files(image_name_label_list,train_ratio,val_ratio):
    ...
    for i in range(len(save_data_list)):
        with open(os.path.join(txt_save_folder,save_data_name[i]), 'w+',encoding="utf8") as fid:
...
if __name__=="__main__":
    ...
    txt_save_folder="/git/PaDiM-master/assets_new_new/data/2021-03-05/json_for_classification/data_prepare_for_semi_supervised_learning"
    choose_labels=[0,5]
    label_mapping=label_mapping(choose_labels)
    image_name_label_list=get_image_label_list_from_origignal_annotation(image_file_path,txt_save_folder,label_mapping)
    get_train_val_test_dataset_from_the_classification_files(image_name_label_list,0.7,0.333)
