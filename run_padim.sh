...
threshold_coefficient=2

for i in {1...20}
do 
for product_id in "0808QFN-16L" "DFN-002A" "QFN-3X3-16L"
do
    python3  main_padim.py --train_num_samples  1500  --test_num_samples 1500 --product_class $product_id  --threshold_coefficient $threshold_coefficient --save_path ./kangqiang_result/threshold_coefficient_$threshold_coefficient 2>&1 |tee Logs/$timestamp.log
done
done
...
do
    python3  main_padim.py  --train_num_samples  0  --test_num_samples 0  --product_class $product_id  2>&1 |tee Logs/$timestamp.log
done
COMMNET